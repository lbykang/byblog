<template>
  <div class="data-container">
    ...
    <div class="content" v-if="!isError">
      <template>
        <div  v-for="(item,index) in qsItem.question" :key="index" class="content-item">
          <div class="item-left">
            <p>{{item.num}} &nbsp; {{item.title}}</p>
              <p class="option" v-for="option in item.options" :key="option">{{option}}</p>
          </div>
          <div class="item-right" v-if="item.type === 'radio'">
            <p>数据占比</p>
            <template >
              <div v-for="(option, index) in item.options" :key="option" class="progress-bar">
